...
bool Zstd::compress(std::vector<uint8_t> &dest, const std::vector<uint8_t> &src, int level) const {
    // 圧縮後サイズの最悪値を取得する
...
bool Zstd::decompress(std::vector<uint8_t> &dest, const std::vector<uint8_t> &src) const {
    // 伸長後のサイズを取得する
